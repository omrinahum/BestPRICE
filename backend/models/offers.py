...
from sqlalchemy import (
    Column, Integer, String, Numeric, DateTime, JSON, UniqueConstraint, Index, ForeignKey
)
...
class Offer(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)                      # offer id
    ...
    seller = Column(String(100), nullable=True, index=True)
    ...
    __table_args__ = (
        ...
        Index("idx_offers_last_seen_at", "last_seen_at"),
    )

    # Relationships
    price_history = relationship("PriceHistory", back_populates="offer", cascade="all, delete-orphan")
    search_links = relationship("SearchOfferLink", back_populates="offer")